...
public class CourierSelector {
    ...
    public CourierSelector(List<BikeTimeCalculator> bikesDelivery, List<CarTimeCalculator> carsDelivery, List<HumanTimeCalculator> humansDelivery) {
        bikes = bikesDelivery;
        cars = carsDelivery;
    ...
    private DeliveryInfo getCarDelivery(int distance) {
        ...
        cars.remove(0);
        return new DeliveryInfo(carDelivery.getCourierName(), carDelivery.getDeliveryTime(distance));
    }
    ...
    private DeliveryInfo getBikeDelivery(int distance) {
        ...
        bikes.remove(0);
        return new DeliveryInfo(bikeDelivery.getCourierName(), bikeDelivery.getDeliveryTime(distance));
    }
    ...
    private DeliveryInfo getHumanDelivery(int distance) {
        ...
        humans.remove(0);
        return new DeliveryInfo(humanDelivery.getCourierName(), humanDelivery.getDeliveryTime(distance));
    }
}