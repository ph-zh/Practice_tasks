/*
Вы продолжаете работать с массивом валют. В коде отражено, что пользователь может выбрать любую из
валют, а затем сообщить программе свой выбор. Это происходит с помощью индексов элементов.
Пользователю сообщаются индексы и предлагается ввести номер того, который соответствует валюте,
которую он хочет заменить. Потом программа присваивает элементу с этим индексом новое значение.
Запустите код несколько раз и попробуйте поменять базовые валюты на любые другие.
 */
package introduction.javaBasics_array.task5;
